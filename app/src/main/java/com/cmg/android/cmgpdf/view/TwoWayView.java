...
package com.cmg.android.cmgpdf.view;
...
public class TwoWayView extends AdapterView<ListAdapter> implements
	...
	public TwoWayView(Context context, AttributeSet attrs, int defStyle) {
		...
		ViewCompat.setOverScrollMode(this,
				ViewCompat.OVER_SCROLL_IF_CONTENT_SCROLLS);

		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.TwoWayView, defStyle, 0);
		...
		int orientation = a.getInt(R.styleable.TwoWayView_android_orientation,
				-1);
		if (orientation >= 0) {
		...
		int choiceMode = a
				.getInt(R.styleable.TwoWayView_android_choiceMode, -1);
		if (choiceMode >= 0) {
	...
	protected void onFocusChanged(boolean gainFocus, int direction,
		...
		if (adapter != null && gainFocus && previouslyFocusedRect != null) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (distance < minDistance) {
					...
					closetChildIndex = i;
					closestChildStart = (mIsVertical ? other.getTop() : other
							.getLeft());
				}
			}
		}
		...
		if (closetChildIndex >= 0) {
			setSelectionFromOffset(closetChildIndex + mFirstPosition,
					closestChildStart);
		} else {
			requestLayout();
		}
	...
	@TargetApi(9)
	private boolean overScrollByInternal(int deltaX, int deltaY, int scrollX,
	...
	protected int computeVerticalScrollOffset() {
		...
		int childHeight = child.getHeight();
		if (childHeight > 0) {
			return Math.max(firstPosition * 100 - (childTop * 100)
					/ childHeight, 0);
		}
	...
	protected int computeHorizontalScrollOffset() {
		...
		int childWidth = child.getWidth();
		if (childWidth > 0) {
			return Math.max(firstPosition * 100 - (childLeft * 100)
					/ childWidth, 0);
		}
	...
	protected int computeVerticalScrollRange() {
		...
		if (mIsVertical && mOverScroll != 0) {
			...
			result += Math.abs((int) ((float) mOverScroll / getHeight()
					* mItemCount * 100));
		}
	...
	protected int computeHorizontalScrollRange() {
		...
		if (!mIsVertical && mOverScroll != 0) {
			// Compensate for overscroll
			result += Math.abs((int) ((float) mOverScroll / getWidth()
					* mItemCount * 100));
		}
	...
	public boolean showContextMenuForChild(View originalView) {
		...
		if (longPressPosition >= 0) {
			...
			if (listener != null) {
				handled = listener.onItemLongClick(TwoWayView.this,
			...
			if (index < 0) {
				Log.e(LOGTAG,
						"onInterceptTouchEvent could not find pointer with id "
								+ mActivePointerId
								+ " - did TwoWayView receive an inconsistent "
								+ "event stream?");
				return false;
			}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (mTouchMode == TOUCH_MODE_FLINGING) {
				...
				return true;
			} else if (mMotionPosition >= 0
					&& mAdapter.isEnabled(mMotionPosition)) {
				mTouchMode = TOUCH_MODE_DOWN;
				triggerCheckForTap();
			}
		...
		case MotionEvent.ACTION_UP: {
			...
			case TOUCH_MODE_DONE_WAITING: {
				...
				if (mIsVertical) {
					inList = x > getPaddingLeft()
							&& x < getWidth() - getPaddingRight();
				} else {
					inList = y > getPaddingTop()
			...
			case TOUCH_MODE_DRAGGING:
				...
				if (Math.abs(velocity) >= mFlingVelocity) {
					...
					mScroller.fling(0, 0, (int) (mIsVertical ? 0 : velocity),
			...
			if (mStartEdge != null && mEndEdge != null) {
				needsInvalidate |= mStartEdge.onRelease()
						| mEndEdge.onRelease();
			}
	...
	public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
		...
		AccessibilityNodeInfoCompat infoCompat = new AccessibilityNodeInfoCompat(
				info);

		if (isEnabled()) {
			if (getFirstVisiblePosition() > 0) {
				infoCompat
						.addAction(AccessibilityNodeInfoCompat.ACTION_SCROLL_BACKWARD);
			}

			if (getLastVisiblePosition() < getCount() - 1) {
				infoCompat
						.addAction(AccessibilityNodeInfoCompat.ACTION_SCROLL_FORWARD);
			}
		}
	}
	...
	public boolean performAccessibilityAction(int action, Bundle arguments) {
		...
		case AccessibilityNodeInfo.ACTION_SCROLL_FORWARD:
			if (isEnabled() && getLastVisiblePosition() < getCount() - 1) {
				...
				if (mIsVertical) {
					viewportSize = getHeight() - getPaddingTop()
							- getPaddingBottom();
				} else {
					viewportSize = getWidth() - getPaddingLeft()
							- getPaddingRight();
				}
	...
	private void forceValidFocusDirection(int direction) {
		if (mIsVertical && direction != View.FOCUS_UP
				&& direction != View.FOCUS_DOWN) {
			throw new IllegalArgumentException(
					"Focus direction must be one of"
							+ " {View.FOCUS_UP, View.FOCUS_DOWN} for vertical orientation");
		} else if (!mIsVertical && direction != View.FOCUS_LEFT
	...
	private void forceValidInnerFocusDirection(int direction) {
		if (mIsVertical && direction != View.FOCUS_LEFT
				&& direction != View.FOCUS_RIGHT) {
			throw new IllegalArgumentException(
					"Direction must be one of"
							+ " {View.FOCUS_LEFT, View.FOCUS_RIGHT} for vertical orientation");
		} else if (!mIsVertical && direction != View.FOCUS_UP
				&& direction != View.FOCUS_DOWN) {
			throw new IllegalArgumentException(
					"direction must be one of"
		...
		if (selectedView != null && selectedView.hasFocus()) {
			...
			if (direction == View.FOCUS_DOWN || direction == View.FOCUS_RIGHT) {
				...
				if (selectedView != null) {
					selectedStart = (mIsVertical ? selectedView.getTop()
				...
				if (selectedView != null) {
					selectedEnd = (mIsVertical ? selectedView.getBottom()
		...
		if (newFocus != null) {
			...
			if (mSelectedPosition != INVALID_POSITION
				...
				if (selectablePosition != INVALID_POSITION
						&& ((movingForward && selectablePosition < positionOfNewFocus) || (movingBackward && selectablePosition > positionOfNewFocus))) {
					return null;
				}
			}
	...
	private int getArrowScrollPreviewLength() {
		...
		int fadingEdgeLength = (mIsVertical ? getVerticalFadingEdgeLength()
				: getHorizontalFadingEdgeLength());

		return mItemMargin
				+ Math.max(MIN_SCROLL_PREVIEW_PIXELS, fadingEdgeLength);
	}
	...
	private boolean arrowScrollImpl(int direction) {
		...
		if (amountToScroll > 0) {
			trackMotionScroll(direction == View.FOCUS_UP
					|| direction == View.FOCUS_LEFT ? amountToScroll
					: -amountToScroll);
			needToRedraw = true;
		}
	...
	private int amountToScroll(int direction, int nextSelectedPosition) {
		...
		if (direction == View.FOCUS_DOWN || direction == View.FOCUS_RIGHT) {
			...
			final int viewToMakeVisibleStart = (mIsVertical ? viewToMakeVisible
					.getTop() : viewToMakeVisible.getLeft());
			final int viewToMakeVisibleEnd = (mIsVertical ? viewToMakeVisible
					.getBottom() : viewToMakeVisible.getRight());
			...
			if (mFirstPosition + numChildren == mItemCount) {
				...
				final int lastChildEnd = (mIsVertical ? lastChild.getBottom()
						: lastChild.getRight());
			...
			if (mFirstPosition == 0) {
				...
				final int firstChildStart = (mIsVertical ? firstChild.getTop()
						: firstChild.getLeft());
	...
	private int amountToScrollToNewFocus(int direction, View newFocus,
		...
		if (direction == View.FOCUS_UP || direction == View.FOCUS_LEFT) {
			final int start = (mIsVertical ? getPaddingTop() : getPaddingLeft());
			final int newFocusStart = (mIsVertical ? mTempRect.top
					: mTempRect.left);
			...
			final int end = (mIsVertical ? getHeight() - getPaddingBottom()
					: getWidth() - getPaddingRight());
			final int newFocusEnd = (mIsVertical ? mTempRect.bottom
					: mTempRect.right);
	...
	private boolean handleKeyEvent(int keyCode, int count, KeyEvent event) {
		...
		if (action != KeyEvent.ACTION_UP) {
			...
			case KeyEvent.KEYCODE_SPACE:
				if (KeyEventCompat.hasNoModifiers(event)) {
					handled = resurrectSelectionIfNeeded()
							|| pageScroll(mIsVertical ? View.FOCUS_DOWN
									: View.FOCUS_RIGHT);
				} else if (KeyEventCompat.hasModifiers(event,
						KeyEvent.META_SHIFT_ON)) {
					handled = resurrectSelectionIfNeeded()
							|| fullScroll(mIsVertical ? View.FOCUS_UP
									: View.FOCUS_LEFT);
				}
			...
			case KeyEvent.KEYCODE_PAGE_UP:
				if (KeyEventCompat.hasNoModifiers(event)) {
					handled = resurrectSelectionIfNeeded()
							|| pageScroll(mIsVertical ? View.FOCUS_UP
									: View.FOCUS_LEFT);
				} else if (KeyEventCompat.hasModifiers(event,
			...
			case KeyEvent.KEYCODE_PAGE_DOWN:
				if (KeyEventCompat.hasNoModifiers(event)) {
					handled = resurrectSelectionIfNeeded()
						...
						KeyEvent.META_ALT_ON)) {
					handled = resurrectSelectionIfNeeded()
							|| fullScroll(mIsVertical ? View.FOCUS_DOWN
									: View.FOCUS_RIGHT);
				}
				break;
		...
		case KeyEvent.ACTION_UP:
			...
			if (isClickable() && isPressed() && mSelectedPosition >= 0
				...
				final View child = getChildAt(mSelectedPosition
						- mFirstPosition);
				if (child != null) {
	...
	private void handleDragChange(int delta) {
		...
		if (mMotionPosition >= 0) {
			...
			// pick something in the middle to make a best guess at things
			// below.
		...
		if (motionView != null) {
			motionViewPrevStart = (mIsVertical ? motionView.getTop()
					: motionView.getLeft());
		}
		...
		if (motionView != null) {
			final int motionViewRealStart = (mIsVertical ? motionView.getTop()
					: motionView.getLeft());

			if (atEdge) {
				final int overscroll = -delta
						- (motionViewRealStart - motionViewPrevStart);
	...
	private void updateOverScrollState(int delta, int overscroll) {
		overScrollByInternal((mIsVertical ? 0 : overscroll),
				...
				(mIsVertical ? mOverScroll : 0), 0, 0, (mIsVertical ? 0
						: mOverscrollDistance),
				(mIsVertical ? mOverscrollDistance : 0), true);
		...
		if (overscrollMode == ViewCompat.OVER_SCROLL_ALWAYS
			...
			float pull = (float) overscroll
					/ (mIsVertical ? getHeight() : getWidth());
			if (delta > 0) {
	...
	private void handleOverScrollChange(int delta) {
		...
		if (delta != 0) {
			...
			// We did not scroll the full amount. Treat this essentially like
			// the
	...
	boolean trackMotionScroll(int incrementalDelta) {
		...
		final int end = (mIsVertical ? getHeight() - paddingBottom : getWidth()
				- paddingRight);
		...
		if (down) {
			...
			for (int i = 0; i < childCount; i++) {
				final View child = getChildAt(i);
				final int childEnd = (mIsVertical ? child.getBottom() : child
						.getRight());
			...
			for (int i = childCount - 1; i >= 0; i--) {
				final View child = getChildAt(i);
				final int childStart = (mIsVertical ? child.getTop() : child
						.getLeft());
		...
		if (spaceBefore < absIncrementalDelta
				|| spaceAfter < absIncrementalDelta) {
			fillGap(down);
		}
		...
		if (!stopped && !mScroller.isFinished()) {
			...
			if (stopped) {
				...
				if (overScrollMode != ViewCompat.OVER_SCROLL_NEVER) {
					final EdgeEffectCompat edge = (diff > 0 ? mStartEdge
							: mEndEdge);

					boolean needsInvalidate = edge.onAbsorb(Math
							.abs((int) getCurrVelocity()));
	...
	private void positionSelector(int position, View selected) {
		...
		mSelectorRect.set(selected.getLeft(), selected.getTop(),
				selected.getRight(), selected.getBottom());
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mItemCount > 0
			...
			final int secondaryMeasureSpec = (mIsVertical ? widthMeasureSpec
					: heightMeasureSpec);
	...
	private void layoutChildren() {
		...
		try {
			...
			if (mItemCount == 0) {
				...
				throw new IllegalStateException(
						"The content of the adapter has changed but "
								...
								+ "from the UI thread. [in TwoWayView("
								+ getId() + ", " + getClass()
			...
			case LAYOUT_SET_SELECTION:
				if (newSelected != null) {
					final int newSelectedStart = (mIsVertical ? newSelected
							.getTop() : newSelected.getLeft());
			...
			case LAYOUT_MOVE_SELECTION:
				selected = moveSelection(oldSelected, newSelected, delta,
						start, end);
				break;
				...
				if (childCount == 0) {
					...
					if (mSelectedPosition >= 0
						...
						if (oldFirstChild != null) {
							offset = (mIsVertical ? oldFirstChild.getTop()
									: oldFirstChild.getLeft());
						}
			...
			if (selected != null) {
				if (mItemsCanFocus && hasFocus() && !selected.hasFocus()) {
					final boolean focusWasTaken = (selected == focusLayoutRestoreDirectChild
							&& focusLayoutRestoreView != null && focusLayoutRestoreView
								.requestFocus()) || selected.requestFocus();
				...
				mSelectedStart = (mIsVertical ? selected.getTop() : selected
						.getLeft());
			} else {
				if (mTouchMode > TOUCH_MODE_DOWN
						&& mTouchMode < TOUCH_MODE_DRAGGING) {
	...
	private View moveSelection(View oldSelected, View newSelected, int delta,
		...
		final int oldSelectedStart = (mIsVertical ? oldSelected.getTop()
				: oldSelected.getLeft());
		final int oldSelectedEnd = (mIsVertical ? oldSelected.getBottom()
				: oldSelected.getRight());
		...
		if (delta > 0) {
			...
			selected = makeAndAddView(selectedPosition, oldSelectedEnd
					+ itemMargin, true, true);

			final int selectedStart = (mIsVertical ? selected.getTop()
					: selected.getLeft());
			final int selectedEnd = (mIsVertical ? selected.getBottom()
					: selected.getRight());
			...
			if (newSelected != null) {
				...
				final int newSelectedStart = (mIsVertical ? newSelected
						.getTop() : newSelected.getLeft());
				selected = makeAndAddView(selectedPosition, newSelectedStart,
						true, true);
			} else {
				// If (A) was not on screen and so did not have a view, position
				// it above the oldSelected (B)
				selected = makeAndAddView(selectedPosition, oldSelectedStart,
						false, true);
			}
			...
			selected = makeAndAddView(selectedPosition, oldSelectedStart, true,
					true);
	...
	private void handleDataChanged() {
		if (mChoiceMode.compareTo(ChoiceMode.NONE) != 0 && mAdapter != null
				&& mAdapter.hasStableIds()) {
	...
	boolean resurrectSelection() {
		...
		if (toPosition >= firstPosition
			...
			selectedStart = (mIsVertical ? selected.getTop() : selected
					.getLeft());
		...
		if (selectedPosition >= firstPosition
				&& selectedPosition <= getLastVisiblePosition()) {
			mLayoutMode = LAYOUT_SPECIFIC;
			updateSelectorState();
	...
	private void measureScrapChild(View scrapChild, int position,
	 ...
	 */
	private int measureWidthOfChildren(int heightMeasureSpec,
	...
	private void setupChild(View child, int position, int top, int left,
		...
		final boolean isPressed = touchMode > TOUCH_MODE_DOWN
	...
	void fillGap(boolean down) {
		...
		if (down) {
			...
			final int offset = (childCount > 0 ? lastEnd + mItemMargin
					: paddingStart);
	...
	private View fillFromSelection(int selectedTop, int start, int end) {
		...
		final int selectedStart = (mIsVertical ? selected.getTop() : selected
				.getLeft());
		final int selectedEnd = (mIsVertical ? selected.getBottom() : selected
				.getRight());
	...
	private void correctTooHigh(int childCount) {
		...
		int firstStart = (mIsVertical ? firstChild.getTop() : firstChild
				.getLeft());

		// Make sure we are 1) Too high, and 2) Either there are more rows above
		// the
		// first row or the first row is scrolled off the top of the drawable
		// area
		if (endOffset > 0 && (mFirstPosition > 0 || firstStart < start)) {
			...
			if (mFirstPosition > 0) {
				firstStart = (mIsVertical ? firstChild.getTop() : firstChild
						.getLeft());
	...
	private void correctTooLow(int childCount) {
		...
		// This is how far the start edge of the first view is from the start of
		// the
		// drawable area
	...
	private void rememberSyncState() {
		...
		if (mSelectedPosition >= 0) {
			...
			if (child != null) {
				mSpecificStart = (mIsVertical ? child.getTop() : child
						.getLeft());
			}
	...
	private ContextMenuInfo createContextMenuInfo(View view, int position,
			long id) {
	...
	private void updateOnScreenCheckedViews() {
		...
		final boolean useActivated = getContext().getApplicationInfo().targetSdkVersion >= Build.VERSION_CODES.HONEYCOMB;
	...
	class RecycleBin {
		...
		void fillActiveViews(int childCount, int firstActivePosition) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				// Note: We do place AdapterView.ITEM_VIEW_TYPE_IGNORE in active
				// views.
		...
		void addScrapView(View scrap, int position) {
			...
			// FIXME: Unfortunately, ViewCompat.setAccessibilityDelegate()
			// doesn't accept
			// null delegates.
			if (Build.VERSION.SDK_INT >= 14) {
		...
		void scrapActiveViews() {
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				if (victim != null) {
					...
					// FIXME: Unfortunately,
					// ViewCompat.setAccessibilityDelegate() doesn't accept
					// null delegates.
					if (Build.VERSION.SDK_INT >= 14) {
		...
		View retrieveFromScrap(ArrayList<View> scrapViews, int position) {
			...
			for (int i = 0; i < size; i++) {
				...
				final LayoutParams lp = (LayoutParams) scrapView
						.getLayoutParams();
	...
	static class SavedState extends BaseSavedState {
		...
		public String toString() {
			return "TwoWayView.SavedState{"
					...
					+ " viewStart=" + viewStart + " height=" + height
	...
	private class PerformClick extends WindowRunnnable implements Runnable {
		...
		public void run() {
			...
			if (adapter != null && mItemCount > 0
				...
				final View child = getChildAt(motionPosition - mFirstPosition);
				if (child != null) {
					performItemClick(child, motionPosition,
		...
		public void run() {
			...
			if (child != null) {
				...
				if (sameWindow() && !mDataChanged) {
					handled = performLongPress(child, motionPosition,
							longPressId);
				}
	...
	private class ListItemAccessibilityDelegate extends
