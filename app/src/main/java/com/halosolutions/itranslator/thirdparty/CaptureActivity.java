...
import com.halosolutions.itranslator.thirdparty.camera.CameraManager;
import com.halosolutions.itranslator.thirdparty.camera.ShutterButton;
import com.halosolutions.itranslator.thirdparty.language.LanguageCodeHelper;
import com.halosolutions.itranslator.thirdparty.language.TranslateAsyncTask;
...
public class CaptureActivity extends Activity implements SurfaceHolder.Callback,
    ...
    void onShutterButtonPressContinuous() {
        ...
        if (lastResult != null) {
            ...
            Toast toast = Toast.makeText(this, "Recognizer failed. Please try again.", Toast.LENGTH_SHORT);
    ...
    private void initOcrEngine(File storageRoot, String languageCode, String languageName) {
        ...
        if (ocrEngineMode != TessBaseAPI.OEM_CUBE_ONLY) {
            for (String s : CUBE_REQUIRED_LANGUAGES) {
                if (s.equals(languageCode)) {
                    ...
                    prefs.edit().putString(PrefFragment.KEY_OCR_ENGINE_MODE, getOcrEngineModeName()).commit();
                }
            }
        }
        ...
        indeterminateDialog = new ProgressDialog(this, AlertDialog.THEME_HOLO_LIGHT);
        indeterminateDialog.setTitle("Please wait");
        String ocrEngineModeName = getOcrEngineModeName();
        if (ocrEngineModeName.equals("Both")) {
            indeterminateDialog.setMessage("Initializing Cube and Tesseract Recognizer engines for " + languageName + "...");
        } else {
            indeterminateDialog.setMessage("Initializing " + ocrEngineModeName + " Recognizer engine for " + languageName + "...");
        }
        ...
        if (ocrEngineMode == TessBaseAPI.OEM_CUBE_ONLY || ocrEngineMode == TessBaseAPI.OEM_TESSERACT_CUBE_COMBINED) {
            ...
            SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
            prefs.edit().putBoolean(PrefFragment.KEY_CONTINUOUS_PREVIEW, false);
        }
    ...
    void handleOcrContinuousDecode(OcrResult ocrResult) {
        ...
        //TranslateAsyncTask asyncTask = new TranslateAsyncTask(this, DEFAULT_SOURCE_LANGUAGE_CODE, DEFAULT_TARGET_LANGUAGE_CODE, ocrResult.getText());
        //asyncTask.execute();
    ...
    void showLanguageName() {
        Toast toast = Toast.makeText(this, "Recognizer: " + sourceLanguageReadable, Toast.LENGTH_LONG);
        toast.setGravity(Gravity.TOP, 0, 0);
        toast.show();
    }
    ...
    void setStatusViewForContinuous() {
        viewfinderView.removeResultText();
        if (CONTINUOUS_DISPLAY_METADATA) {
            statusViewBottom.setText("Recognizer: " + sourceLanguageReadable + " - waiting...");
        }
    }
    ...
    private boolean checkFirstLaunch() {
        try {
            ...
            SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
            int lastVersion = prefs.getInt(PrefFragment.KEY_HELP_VERSION_SHOWN, 0);
            if (lastVersion == 0) {
                isFirstLaunch = true;
            } else {
                isFirstLaunch = false;
            ...
            if (currentVersion > lastVersion) {
                ...
                prefs.edit().putInt(PrefFragment.KEY_HELP_VERSION_SHOWN, currentVersion).commit();
                return true;
            }
    ...
    private void retrievePreferences() {
        ...
        setSourceLanguage(prefs.getString(PrefFragment.KEY_SOURCE_LANGUAGE_PREFERENCE, CaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE));
        setTargetLanguage(prefs.getString(PrefFragment.KEY_TARGET_LANGUAGE_PREFERENCE, CaptureActivity.DEFAULT_TARGET_LANGUAGE_CODE));
        isTranslationActive = prefs.getBoolean(PrefFragment.KEY_TOGGLE_TRANSLATION, false);

        // Retrieve from preferences, and set in this Activity, the capture mode preference
        if (prefs.getBoolean(PrefFragment.KEY_CONTINUOUS_PREVIEW, CaptureActivity.DEFAULT_TOGGLE_CONTINUOUS)) {
        ...
        String pageSegmentationModeName = prefs.getString(PrefFragment.KEY_PAGE_SEGMENTATION_MODE, pageSegmentationModes[0]);
        ...
        String[] ocrEngineModes = getResources().getStringArray(R.array.ocrenginemodes);
        String ocrEngineModeName = prefs.getString(PrefFragment.KEY_OCR_ENGINE_MODE, ocrEngineModes[0]);
    ...
    private void setDefaultPreferences() {
        ...
        // Continuous preview
        prefs.edit().putBoolean(PrefFragment.KEY_CONTINUOUS_PREVIEW, CaptureActivity.DEFAULT_TOGGLE_CONTINUOUS).commit();

        // Recognition language
        prefs.edit().putString(PrefFragment.KEY_SOURCE_LANGUAGE_PREFERENCE, CaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE).commit();

        // Translation
        prefs.edit().putBoolean(PrefFragment.KEY_TOGGLE_TRANSLATION, CaptureActivity.DEFAULT_TOGGLE_TRANSLATION).commit();

        // Translation target language
        prefs.edit().putString(PrefFragment.KEY_TARGET_LANGUAGE_PREFERENCE, CaptureActivity.DEFAULT_TARGET_LANGUAGE_CODE).commit();

        // OCR Engine
        prefs.edit().putString(PrefFragment.KEY_OCR_ENGINE_MODE, CaptureActivity.DEFAULT_OCR_ENGINE_MODE).commit();

        // Autofocus
        prefs.edit().putBoolean(PrefFragment.KEY_AUTO_FOCUS, CaptureActivity.DEFAULT_TOGGLE_AUTO_FOCUS).commit();

        // Disable problematic focus modes
        prefs.edit().putBoolean(PrefFragment.KEY_DISABLE_CONTINUOUS_FOCUS, CaptureActivity.DEFAULT_DISABLE_CONTINUOUS_FOCUS).commit();

        // Beep
        prefs.edit().putBoolean(PrefFragment.KEY_PLAY_BEEP, CaptureActivity.DEFAULT_TOGGLE_BEEP).commit();

        // Page segmentation mode
        prefs.edit().putString(PrefFragment.KEY_PAGE_SEGMENTATION_MODE, CaptureActivity.DEFAULT_PAGE_SEGMENTATION_MODE).commit();

        // Light
        prefs.edit().putBoolean(PrefFragment.KEY_TOGGLE_LIGHT, CaptureActivity.DEFAULT_TOGGLE_LIGHT).commit();
    }
