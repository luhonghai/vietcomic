...
class FancyCoverFlowItemWrapper extends ViewGroup {
	 ...
	 */
	public Bitmap wrappedViewBitmap;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (changed) {
			...
			if (this.wrappedViewBitmap == null
				...
				synchronized (cache) {
					SoftReference<Bitmap> sr = cache.get(Integer.valueOf(pos));
					if (sr != null) {
						Bitmap bm = sr.get();
						...
						cache.remove(Integer.valueOf(pos));
					}
					cache.put(Integer.valueOf(pos), new SoftReference<Bitmap>(
	...
	protected void dispatchDraw(Canvas canvas) {
		if (this.wrappedViewBitmap != null
				&& !this.wrappedViewBitmap.isRecycled()) {
	...
	public void recycle() {
		if (wrappedViewBitmap != null) {
			wrappedViewBitmap.recycle();
			wrappedViewBitmap = null;
		}

	}

	public void setCache(Map<Integer, SoftReference<Bitmap>> cache, int pos) {
		this.pos = pos;
		this.cache = cache;
	}
}
